...
This documentation site is itself an MPP server. Create an account, fund it with testnet tokens, and make a paid request to see the full challenge-credential-receipt flow in action.
...
MPP comes with a suite of official SDKs named `mpay`. The `mpay` SDKs offer high-level abstractions and low-level primitives to implement and extend the Machine Payments Protocol. 
