...
1. **Client requests a resource** – Standard HTTP request (GET, POST, etc.)
...
1. **Method identifier** – Unique lowercase ASCII string (e.g., `tempo`, `stripe`)
...
Servers should use the `Retry-After` header to indicate when clients may retry failed payments.
...
For non-idempotent methods (POST, PUT, DELETE), servers should accept an `Idempotency-Key` header to enable safe client retries.
...
- Servers should keep challenges under 8KB
- Clients must handle challenges of at least 4KB
- Servers must handle credentials of at least 4KB

### Internationalization

- All string values use UTF-8 encoding
- Payment method identifiers are restricted to ASCII lowercase
- The `realm` parameter should use ASCII-only values
- The `description` parameter may contain localized text; servers should use `Accept-Language` to determine appropriate language
