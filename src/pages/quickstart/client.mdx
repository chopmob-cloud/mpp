...
Call `Mpay.create` at startup. This polyfills `globalThis.fetch` so all `fetch` calls automatically handle `402` payment challenges.
...
Set `polyfill: false` and use the returned `fetch`:
