...
`pget` is a non-interactive commandline tool for making HTTP requests with automatic payment support. Handles `402 Payment Required` responses with built-in payment methods and permissions. Designed for easy use from scripts, cron jobs, and AI agents.
...
The agent simply calls pget like any other commandline tool when it detects a `402` Payment Required response. Payment is handled transparently.
...
- [Examples](/tools/pget/examples) — More usage patterns and real endpoints
- [Reference](/tools/pget) — Full command reference and configuration
