...
Use [`tempo`](/sdk/typescript/server/Method.tempo) to accept payment sessions. The server needs an RPC URL for on-chain verification during channel open/close, and a storage backend for channel state.
...
:::info
See [`tempo.stream` server reference](/sdk/typescript/server/Method.tempo.session) for the full parameter list.
:::
...
Use [`tempo`](/sdk/typescript/client/Method.tempo) with `Mpay.create` to sign vouchers automatically when the server requests payment sessions.
...
:::info
See [`tempo.stream` client reference](/sdk/typescript/client/Method.tempo.session) for the full parameter list.
:::
...
<Cards>
  <SpecCard to="/specs/draft-tempo-session-00" />
</Cards>
